...
const fetchProduct = async (code: string, type: "barcode" | "barCodenumber") => {
  try {
    const res = await fetch(
      `http://localhost:5000/products/bar-code?${type}=${code}`,
      { credentials: "include" } // send cookies for cookieAuth
    );
...
const [showScanner, setShowScanner] = useState(false);
  ...
  // Add product manually (by barCodenumber)
  ...
  // Add product via scanner (barcode)
